...
**Defense:** Artistry's counter-narrative says "a migration that runs exactly once — intentionally dropping a table column." This is WRONG. Migration file `20251210231153_move_to_version_id/migration.sql:18` has `DELETE FROM "Object"` with no WHERE clause. This is NOT auto-generated Prisma output — it's a raw SQL migration step that was manually added (Prisma generates ALTER TABLE, not full-table DELETE). If this migration re-runs or was never applied, ALL Object rows are deleted. **P0 survives, severity unchanged.**
