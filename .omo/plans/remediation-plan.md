...
| P1-1 | Fix Promise boolean at `session/index.ts:195` | `server/server/internal/session/index.ts:195` | 30m | Add unit test: mock `removeSession` to return false → assert `signout` returns false AND cookie NOT cleared. `pnpm --filter drop test` | None |
