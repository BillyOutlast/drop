...
interface PCGamingWikiGame extends PCGamingWikiSearchStub {
  ...
  // TODO(sonar): persist Series field as a game tag - deferred, needs tag system integration
  Series: string | null;
...
export class PCGamingWikiProvider implements MetadataProvider {
  ...
  private async getPageContent(pageID: string) {
    ...
    if (receptionEle.length > 0) {
      ...
      // TODO(sonar): refactor reception rating parsing into smaller functions - deferred, complex nested logic
  ...
  async fetchCompany({
    ...
    // TODO(sonar): replace jdenticon fallback with actual company logo - deferred, needs logo API integration
